...
/**
 * The {@link HomematicIPEnergySensorsInterface} is responsible for handling commands, which are sent to one of the
 * channels.
 *
...
public class HomematicIPEnergySensorsInterface extends HomematicIPAbstractDeviceHandler {
    ...
    private void updateSensorChannel(ChannelUID channelUID, JsonObject data) {
        ...
        switch (channelId) {
            ...
            case CHANNEL_ENERGY_ONE:
                JsonElement energyCounterOne = data.get(PROPERTY_ENERGY_COUNTER_ONE);
                if (energyCounterOne.isJsonPrimitive()) {
                    state = HomematicIPUtils.getQuantityTypeState(energyCounterOne.getAsDouble(), Units.KILOWATT_HOUR);
                }
                break;
            case CHANNEL_ENERGY_TWO:
                JsonElement energyCounterTwo = data.get(PROPERTY_ENERGY_COUNTER_TWO);
                if (energyCounterTwo.isJsonPrimitive()) {
                    state = HomematicIPUtils.getQuantityTypeState(energyCounterTwo.getAsDouble(), Units.KILOWATT_HOUR);
                }
                break;
            case CHANNEL_ENERGY_THREE:
                JsonElement energyCounterThree = data.get(PROPERTY_ENERGY_COUNTER_THREE);
                if (energyCounterThree.isJsonPrimitive()) {
                    state = HomematicIPUtils.getQuantityTypeState(energyCounterThree.getAsDouble(),
                            Units.KILOWATT_HOUR);
                }
                break;
        }
