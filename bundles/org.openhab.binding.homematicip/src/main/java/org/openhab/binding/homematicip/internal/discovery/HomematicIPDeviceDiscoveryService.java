...
public class HomematicIPDeviceDiscoveryService
    ...
    public void onDeviceAdded(String deviceId, JsonObject device) {
        ...
        switch (type) {
            case ENERGY_SENSORS_INTERFACE:
                ...
                JsonElement connectedEnergySensorType = functionalChannels.get(FUNCTIONAL_TYPE_CHANNEL_DEVICE_CHANNEL)
                        .getAsJsonObject().get(PROPERTY_CONNECTED_ENERGY_SENSOR_TYPE);
    ...
    public void onGroupAdded(String groupId, JsonObject group) {
        ...
        if (thingTypeUID != null) {
            ...
            DiscoveryResult discoveryResult = DiscoveryResultBuilder.create(thingUID).withProperties(properties)
                    .withRepresentationProperty(THING_PROPERTY_ID).withBridge(bridgeUID).withLabel(discoveryResultLabel)
                    .build();
